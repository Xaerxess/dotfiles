...
case "${prev}" in
    linode-cli | linode | lin)
        COMPREPLY=( $(compgen -W "account betas events payment-methods service-transfers users databases domains images linodes kernels stackscripts lke longview managed networking firewalls vlans nodebalancers object-storage profile security-questions sshkeys phone regions tickets tags volumes vpcs --help" -- ${cur}) )
        return 0
        ;;
    account)
